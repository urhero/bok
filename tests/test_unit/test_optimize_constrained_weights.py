...
class TestSimulateConstrainedWeightsStyleCap:
    ...
    def test_test_mode_relaxes_style_cap(
        ...
        # test_mode=True
        _, weights_tbl_test = optimize_constrained_weights(
            rtn_df=rtn_df,
            style_list=style_list,
            mode="monte_carlo",
            num_sims=1000,
...
class TestSimulateConstrainedWeightsNumSims:
    ...
    def test_more_sims_may_find_better_solution(
        ...
        # 적은 시뮬레이션
        np.random.seed(42)
        best_stats_low, _ = optimize_constrained_weights(
            rtn_df=rtn_df,
            style_list=style_list,
            mode="monte_carlo",
            num_sims=100,
        ...
        # 많은 시뮬레이션
        np.random.seed(42)
        best_stats_high, _ = optimize_constrained_weights(
            rtn_df=rtn_df,
            style_list=style_list,
            mode="monte_carlo",
            num_sims=10000,
...
class TestSimulateConstrainedWeightsBatchProcessing:
    ...
    def test_batch_size_affects_memory_not_result(
        ...
        # 작은 배치
        np.random.seed(42)
        best_stats_small, weights_small = optimize_constrained_weights(
            rtn_df=rtn_df,
            style_list=style_list,
            mode="monte_carlo",
            num_sims=1000,
        ...
        # 큰 배치
        np.random.seed(42)
        best_stats_large, weights_large = optimize_constrained_weights(
            rtn_df=rtn_df,
            style_list=style_list,
            mode="monte_carlo",
            num_sims=1000,
...
class TestSimulateConstrainedWeightsReproducibility:
    ...
    def test_same_seed_same_result(
        ...
        np.random.seed(42)
        best_stats_1, weights_1 = optimize_constrained_weights(
            rtn_df=rtn_df,
            style_list=style_list,
            mode="monte_carlo",
            num_sims=1000,
        ...
        np.random.seed(42)
        best_stats_2, weights_2 = optimize_constrained_weights(
            rtn_df=rtn_df,
            style_list=style_list,
            mode="monte_carlo",
            num_sims=1000,
