# -*- coding: utf-8 -*-
"""백테스트 선정 경로의 프로세스 간 재현성(determinism) 계약 테스트.

배경: 팩터 선정 경로가 Python ``set`` 반복에 의존하면,
...
def test_full_probe_byte_identical_across_hash_seeds(probe_outputs):
    ...
    assert len(set(probe_outputs)) == 1, "선정 경로 출력이 프로세스마다 다름"
