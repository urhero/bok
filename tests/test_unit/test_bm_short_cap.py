...
def test_excess_redistributed_only_when_opted_in():
    """redistribute=True 일 때만 초과분이 여유 숏으로 재분배된다 (비권장 모드)."""
    w = pd.Series({"A": 0.10, "S1": -0.08, "S2": -0.02})
    bm = pd.Series({"A": 0.05, "S1": 0.03, "S2": 0.50})
    out = apply_bm_short_cap(w, bm, target_short=0.10, redistribute=True)
    assert out["S1"] == pytest.approx(-0.03)
    assert out["S2"] == pytest.approx(-0.07)      # 0.10 - 0.03
...
def test_dollar_neutrality_preserved():
    ...
    out = apply_bm_short_cap(w, bm, target_short=0.10)
...
def test_not_in_bm_cannot_be_shorted():
    ...
    out = apply_bm_short_cap(w, bm, target_short=0.05)
...
def test_default_does_not_redistribute():
    """기본 동작: 초과분은 잘라내기만 — 다른 숏으로 밀지 않는다.

    재분배하면 소형주(BM 비중 작음) 숏이 대형주 숏으로 이동해 팩터 신호가
    사이즈 베팅으로 변질된다 (2026-08-21 실측: Sharpe +0.715 -> -0.855).
    """
    w = pd.Series({"A": 0.10, "S1": -0.08, "S2": -0.02})
    bm = pd.Series({"A": 0.05, "S1": 0.03, "S2": 0.50})
    out = apply_bm_short_cap(w, bm, target_short=0.10)     # redistribute 기본 False
