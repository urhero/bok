...
PIPELINE_PARAMS = {
    ...
    "bm_short_cap": True,              # 종목별 숏 비중 <= 그 종목의 BM 비중 (총 보유가 음수가 되지 않게). 배수 적용 후 최종 비중에 적용, 초과분은 여유 있는 숏에 재분배(water-filling), 롱은 숏 총액에 맞춰 비례 조정(중립 유지). BM 미편입 종목은 숏 불가. data/{benchmark}_bmwgt.parquet 필요 (2026-08-21 사용자 지정)
    "mp_target_gross": 0.40,           # MP 배포 목표 총 gross (롱+|숏|). 설정 시 매 시점 배수를 자동 산출해 노출을 고정 (netting 변동 흡수). None 이면 data/mp_multiplier.csv 의 시점별 수동 배수 사용. 0.40 = 롱 +20% / 숏 -20% (2026-08-19 사용자 지정)
