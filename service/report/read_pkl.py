...
from service.live.model_portfolio import (
    OUTPUT_DIR,
...
def plot_factor_returns(
    ...
    if mode == "Factor Return":
        ...
        df_group = (
            data.groupby(["ddt", "quantile"], observed=False)["M_RETURN"]
            .mean()
        ...
        df_mean["long"] = (
            (df_mean["mean"] > df_mean.loc["Q1", "mean"] - thresh).astype(int).cumprod()
        )
        df_mean["short"] = (
            (df_mean["mean"] < df_mean.loc["Q5", "mean"] + thresh).astype(int) * -1
        )
        ...
        bar_colors = [
            (
                BLUE
                if q_label_map.get(q, 0) == 1
                else RED if q_label_map.get(q, 0) == -1 else WHITE
            )
            for q in idx_order
        ]
        ax.bar(
            [str(q) for q in y.index],
            y.values,
            color=bar_colors,
            edgecolor="black",
            linewidth=0.8,
...
def _generate_plots(
    ...
    for i, (style_name, factor_abbr, full_name) in enumerate(
        zip(style_cagr, abbr_cagr, name_cagr)
    ):
        ...
        try:
            if isinstance(data_source, list):
                if factor_abbr in kept_abbr:
                    ...
                    logger.warning(
                        f"Factor {factor_abbr} not in kept_abbr, skipping plot"
                    )
                    continue
                ...
                if factor_abbr in data_source:
                    ...
                    logger.warning(
                        f"Factor {factor_abbr} not in data_source, skipping plot"
                    )
                    continue
            else:
