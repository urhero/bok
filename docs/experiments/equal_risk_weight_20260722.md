...
## 스타일 캡 기준 실험 (2026-07-22 추가: 명목비중 vs 리스크 예산)

"캡도 리스크 기준(w*sigma 예산의 25%)이 논리적으로 일관"이라는 가설로
`style_cap_basis="risk"` 옵션 구현 (캡 재분배를 리스크 공간에서 수행 후 명목비중 환산,
기본 "weight"는 기존 경로 무변경). A/B:

| 케이스 | CAGR | MDD | Sharpe | Calmar |
|--------|------|-----|--------|--------|
| ERW + 명목비중 캡 (기존) | 2.14% | -10.1% | **0.598** | **0.211** |
| ERW + 리스크 캡 | 2.03% | -10.1% | 0.572 | 0.201 |

- 하위구간(전/후반, 2023~) **전 구간에서 명목비중 캡 우위** (전반 Sharpe 0.570 vs 0.513).
- 해석: 리스크 캡은 저변동 스타일의 명목비중 집중을 무제한 허용하는데, trailing sigma가
  못 잡는 리스크(저변동 스타일의 국면 전환/크래시 리스크)까지 집중이 커짐.
  명목비중 캡은 1/sigma 리스크 균형 위에 "명목 분산"이라는 독립적 제약을 더해 상호보완.
